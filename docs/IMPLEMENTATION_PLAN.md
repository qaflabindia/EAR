...
| **4 — Ecosystem** ◐ tools + seam shipped | 11–14 | ToolBinder + LangChain tool binding (3.11 ✅: declared tools meet executables, ReAct deliberation, every invocation on the trail, failures contained; per-call policy denial of a specific tool still open). Deliberator `backend` seam (3.10 ✅ the seam, on the trail; the PydanticAI adapter itself ⏳ open). AutoGen patterns (3.9) ⏳ and LangGraph compile (3.8) ⏳ open — next slices | live: the model called the bound amortization calculator mid-deliberation, on the record ✅; backend-equivalent trails: pending the remaining adapters |
