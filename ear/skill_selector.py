...
LLM-ranked when a model is active and falls back to keyword overlap
offline, exactly like Discoverer -- and it short-circuits (returns every
skill, in order) whenever a persona has no more than `top_k` skills, so the
common case costs nothing and behaviour is unchanged for small personas."""
...
class SkillSelector:
    ...
    by LLM judgment. Returns every skill unchanged when the persona has
    `top_k` or fewer."""
    ...
    def _rank_with_llm(self, skills: list[Skill], intent: Intent, lm: Any) -> list[Skill]:
        ...
        ranker = dspy.Predict(RankRelevantSkills)
        with dspy.context(lm=lm):
            result = ranker(intent_text=intent.text, available_skills=catalogue)
        by_name = {skill.name: skill for skill in skills}
        found = [by_name[name] for name in result.relevant_skill_names if name in by_name]
