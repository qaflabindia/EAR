"""A restricted boolean/arithmetic expression evaluator for Dharma rules.

Dharma rules are short strings such as ``"purchase_amount <= approval_limit"``
supplied by whoever wires up the runtime's policies. They must never be
passed to ``eval``/``exec`` since that would let a policy string execute
arbitrary code. This module walks the parsed AST instead and only permits
literals, names, comparisons, boolean logic and basic arithmetic.
"""
...
class UnsafeExpressionError(ValueError):
    """Raised when a rule contains a construct outside the allowed grammar."""


class MissingVariableError(UnsafeExpressionError):
    """Raised when a rule references a name absent from the given context."""
